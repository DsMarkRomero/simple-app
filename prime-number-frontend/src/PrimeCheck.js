...
const PrimeCheck = () => {
    ...
    const fetchVersion = async () => {
        try {
            const response = await axios.get('http://localhost:5002/api/prime/version');
    ...
    const checkPrime = async () => {
        try {
            const response = await axios.get(`http://localhost:5002/api/prime/isprime/${number}`);
    ...
    return (
        <div>
			...
			<h2> Cambio para activar el trigger v1 en MASTER</h2>
